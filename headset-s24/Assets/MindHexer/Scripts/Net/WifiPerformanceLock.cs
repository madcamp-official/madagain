...
namespace MindHexer.Headset.Net
{
    ...
    ///
    /// **부팅 안전성**: JNI 취득은 <c>Start</c>(첫 프레임)에서 수행한다. 헤드셋 부트스트랩은
    /// <c>BeforeSceneLoad</c>에서 조립되는데, 그 도메인 초기화 시점에 JNI(AndroidJavaClass/Object)를
    /// 호출하면 GameActivity 빌드의 일부 기기에서 초기화 경합/크래시가 날 수 있다. 첫 프레임으로 미루면
    /// 액티비티가 완전히 준비된 뒤 안전하게 실행되며, 실패해도 앱 구동에는 영향이 없다.
    /// </summary>
    public sealed class WifiPerformanceLock : MonoBehaviour
    {
        ...
        // OnEnable(BeforeSceneLoad 도메인 초기화 중)이 아니라 첫 프레임에 취득 → 부팅 임계경로에서 JNI 제거.
