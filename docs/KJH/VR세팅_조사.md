...
> **✅ 구매 확정 (2026-07-25): 엑토 프로 VR-01** (₩26,300, 쿠팡 로켓, 리뷰 2,454).
> - 지원 폰 **≤175×85mm** → S24+(158.5×75.9) **여유 있게 장착**. 초점 조절 O(IPD 조절은 미확인, 초점만일 가능성).
> - **패시브 렌즈 + 폰 홀더** 방식이라 우리 Cardboard 앱이 그 안에서 스테레오로 뜸. Cardboard "공식 호환" 라벨은 없으니 **렌즈 프로파일(간격·왜곡)은 WWGC로 직접 생성**(§2).
> - 케이스는 벗기고 장착 권장. 안 맞으면 30일 무료 반품.
> - (35+ 모델 조사 결과: S24+ 6.7"가 들어가는 건 소수 — 엑토 VR-01·Homido Prime·Shinecon G10·DESTEK V5·VRG Pro X7 등. 프리미엄 Zeiss·Gear VR·Daydream은 크기 초과 또는 전용 방식이라 부적합.)
