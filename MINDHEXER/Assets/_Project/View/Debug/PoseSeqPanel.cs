...
{
    /// <summary>
    /// F3 — 시퀀스 세그먼트 튜닝 패널. 현재 재생 시퀀스의 "포즈 사이 간격"을 구간별로 각각 조절한다.
    /// 스프링·정지·전체 배속까지 여기서 전부. 값 바꾸고 "다시 재생"으로 즉시 확인.
    /// (F1 = 전투, F2 = 재생 전역, F3 = 세그먼트별)
    /// </summary>
    public class PoseSeqPanel : MonoBehaviour
    {
        ...
        void OnGUI()
        {
            ...
            if (GUILayout.Button("<b>이 애니메이션 타이밍 저장</b>", Rich(), GUILayout.Height(26f)))
                Debug.Log(pp.SaveTiming() ? "[F3] 타이밍 저장: " + pp.CurrentKey : "[F3] 저장 실패");
        ...
        static string EaseHint(int type) =>
            type == PosePlayer.EzIn    ? "천천히 시작 → 빠르게 (휘두르기 시작)"
          : type == PosePlayer.EzOut   ? "빠르게 시작 → 천천히 (타격 후 감속)"
