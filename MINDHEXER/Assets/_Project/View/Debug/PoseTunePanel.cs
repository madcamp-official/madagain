...
{
    /// <summary>
    /// F2 — 포즈 재생 튜닝 패널. PosePlayer의 타이밍·스프링 수치를 Play 중 실시간 조절한다.
    /// "다시 재생"으로 마지막 combo/play 를 현재 수치로 즉시 재생 → 보면서 튜닝.
    /// (F1 = 전투 튜닝, F2 = 포즈 재생 튜닝)
    /// </summary>
    public class PoseTunePanel : MonoBehaviour
    {
        ...
        void OnGUI()
        {
            ...
            GUILayout.Label("<b>포즈 재생 튜닝 (F2)</b>", Rich());
            ...
            else
            {
                ...
                if (GUILayout.Button("후퇴량 자동 맞춤", GUILayout.Height(24f)))
                    Debug.Log(vc.AutoFitPullBack() ? $"[F2] 후퇴량 자동 설정 → {vc.pullBack:0.000}m" : "[F2] 측정 실패");
            ...
            {
                if (!pp.Replay()) Debug.Log("[F2] 먼저 콘솔에서 play/combo 를 한 번 실행하십시오.");
            }
            if (GUILayout.Button("정지", GUILayout.Height(26f))) pp.Stop();
            GUILayout.EndHorizontal();

            GUILayout.Label("<size=10>구간별 상세 튜닝·저장은 F3.</size>", Rich());
            GUILayout.EndArea();
        }
