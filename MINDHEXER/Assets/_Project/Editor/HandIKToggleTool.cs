...
{
    /// <summary>
    /// 손 IK를 좌·우 따로 켜고 끈다. 메뉴에 체크 표시가 뜬다.
    ///
    /// 쓰임새:
    ///   · 기본 자세를 잡을 땐 IK를 꺼야 한다(켜져 있으면 매 프레임 덮어써서 조정이 안 먹는다)
    ///   · 오른손 IK = 포즈 잡는 보조. 애니메이션 찍을 땐 끔
    ///   · 왼손 IK = 칼을 계속 따라감. 보통 켜둠
    /// </summary>
    public static class HandIKToggleTool
    {
        const string MenuR   = "Tools/뷰모델/IK 오른손 켜기·끄기 %#r";   // Ctrl+Shift+R
        const string MenuL   = "Tools/뷰모델/IK 왼손 켜기·끄기 %#l";     // Ctrl+Shift+L
        const string MenuOff = "Tools/뷰모델/IK 양손 끄기 %#o";          // Ctrl+Shift+O

        /// <summary>왼손 IK인가. 끝 뼈 이름 우선, 없으면 오브젝트 이름으로 판별.</summary>
