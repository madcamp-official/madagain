...
namespace Game.EditorTools
{
    ...
    /// <summary>
    /// 애니메이션 명세 JSON을 읽어 AnimationClip을 생성한다. (Precog에서 포팅 — 이름 일반화)
    ///
    ...
    ///
    /// ★ 등반(잡고 올라가기)은 이걸로 굽지 않는다 — 모서리 위치·높이·각도가 매번 달라
    ///   고정 프레임으로 표현할 수 없다. 절차적(<see cref="MantleRig"/>)이어야 한다.
    ///   이 도구는 대기·조준·상호작용처럼 <b>고정된</b> 동작에 쓴다.
    /// </summary>
    public class ClipBuildTool : EditorWindow
    {
        /// <summary>포즈·명세 JSON이 있는 폴더. PoseJsonTool과 같은 값이어야 한다.
        /// (상수를 저쪽에서 참조하면 에디터 툴끼리 불필요하게 얽힌다 — 각자 들고 있는다.)</summary>
        ...
        void OnGUI()
        {
            ...
            if (specs.Length == 0)
                EditorGUILayout.HelpBox($"{PoseDir} 에 anim_*.json 이 없습니다.", MessageType.None);
        }
        ...
        void Refresh()
        {
            ...
            specs = Directory.GetFiles(PoseDir, "anim_*.json");
            Array.Sort(specs);
        }
        ...
        void Build(string specFile)
        {
            ...
            {
                string pf = Path.Combine(PoseDir, $"pose_{k.pose}.json");
            ...
            for (int i = 0; i < spec.keys.Length; i++)
            {
                ...
                var p = poses[i];
                if (p.bones != null)
                    foreach (var b in p.bones) Add(rotByPath, b.path, t, PoseMath.ToQ(b.quat));
                if (p.objects != null)
                    foreach (var o in p.objects)
                    {
                        if (o.quat  != null && o.quat.Length  == 4) Add(rotByPath,   o.path, t, PoseMath.ToQ(o.quat));
                        if (o.pos   != null && o.pos.Length   == 3) Add(posByPath,   o.path, t, PoseMath.ToV3(o.pos));
                        if (o.scale != null && o.scale.Length == 3) Add(scaleByPath, o.path, t, PoseMath.ToV3(o.scale));
                    }
            }
