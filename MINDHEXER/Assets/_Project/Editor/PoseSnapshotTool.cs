...
{
    /// <summary>
    /// 포즈 스냅샷 — 현재 팔·손가락·씬뷰 카메라를 이름 붙여 저장하고 언제든 복원한다.
    /// 키포즈를 모아두고 불러가며 Animation 창에 찍는 용도.
    /// (Precog에서 포팅 — 카타나 전용 이름을 일반화. 칼/그립 필드는 스키마 호환을 위해
    ///  PoseSnapshot.cs에 남아있지만 우리 게임에는 해당 오브젝트가 없어 항상 비어있다.
    ///  "그립" 대신 손 IK 타겟(HandTarget_R/L)을 같은 슬롯에 담아 재사용한다.)
    /// </summary>
    public class PoseSnapshotTool : EditorWindow
    {
        ...
        // 복원 시 무엇을 되돌릴지
        bool applyBones = true, applyHandTargets = true, applyCam = true, applyIk = true;
        ...
        void OnGUI()
        {
            EditorGUILayout.HelpBox(
                ...
                "뼈(어깨·팔꿈치·손목·손가락) · 손 IK 타겟 · 씬뷰 카메라를 함께 담습니다.", MessageType.Info);
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                applyHandTargets = GUILayout.Toggle(applyHandTargets, "손타겟",    EditorStyles.miniButtonMid);
            ...
            for (int i = 0; i < asset.slots.Count; i++)
            {
                ...
                using (new EditorGUILayout.VerticalScope("box"))
                {
                    ...
                    EditorGUILayout.LabelField(
                        ...
                        (s.hasGripR ? " · 손타겟R" : "") + (s.hasGripL ? " · 손타겟L" : "") +
        ...
        void Fill(PoseSlot slot)
        {
            ...
            // 손 IK 타겟 (PoseSlot의 grip 필드를 재사용 — 무기 없는 우리 게임 전용 의미)
            Transform hr = FindIn(root, "HandTarget_R"), hl = FindIn(root, "HandTarget_L");
            slot.hasGripR = hr != null;
            if (hr != null) { slot.gripRPos = hr.localPosition; slot.gripRRot = hr.localRotation; }
            slot.hasGripL = hl != null;
            if (hl != null) { slot.gripLPos = hl.localPosition; slot.gripLRot = hl.localRotation; }
            ...
            {
                if (fp.gameObject.name.Contains("Left") || fp.gameObject.name.EndsWith("_L")) slot.fingerGripL = fp.grip; else slot.fingerGripR = fp.grip;
            }
        ...
        void Restore(PoseSlot s)
        {
            ...
            if (applyHandTargets)
            {
                if (s.hasGripR && hr != null) { Undo.RecordObject(hr, "포즈 복원"); hr.localPosition = s.gripRPos; hr.localRotation = s.gripRRot; }
                if (s.hasGripL && hl != null) { Undo.RecordObject(hl, "포즈 복원"); hl.localPosition = s.gripLPos; hl.localRotation = s.gripLRot; }
            }
            if (applyIk && s.hasIk)
            {
                foreach (var ik in root.GetComponentsInChildren<HandIK>(true))
                ...
                foreach (var fp in root.GetComponentsInChildren<FingerPoser>(true))
                {
                    ...
                    fp.grip = (fp.gameObject.name.Contains("Left") || fp.gameObject.name.EndsWith("_L")) ? s.fingerGripL : s.fingerGripR;
                }
            }
